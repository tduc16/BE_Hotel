...
export class AdminRoomCategoriesService {
  ...
  async createCategory(createDto: CreateRoomCategoryDto) {
    ...
    if (existing) {
      throw new ConflictException(
        `Room category name '${createDto.name}' already exists`,
      );
    }
  ...
  async updateCategory(id: string, updateDto: UpdateRoomCategoryDto) {
    ...
    if (updateDto.name && updateDto.name !== category.name) {
      ...
      if (existing) {
        throw new ConflictException(
          `Room category name '${updateDto.name}' already exists`,
        );
      }
    }
    ...
    if (
      updateDto.append_gallery_images &&
      updateDto.append_gallery_images.length > 0
    ) {
    ...
    if (updateDto.base_price !== undefined)
      category.base_price = updateDto.base_price;
    if (updateDto.capacity !== undefined)
      category.capacity = updateDto.capacity;
    if (updateDto.amenities !== undefined)
      category.amenities = updateDto.amenities;
    if (updateDto.is_active !== undefined)
      category.is_active = updateDto.is_active;
    if (updateDto.thumbnail_url !== undefined) {
      // remove old thumbnail if changed
      if (
        category.thumbnail_url &&
        category.thumbnail_url !== updateDto.thumbnail_url
      ) {
