...
export class AdminRoomsService {
  ...
  async createRoom(createDto: CreateRoomDto) {
    ...
    const existingRoom = await this.roomRepo.findOne({ where: { room_number } });
    if (existingRoom) {
    ...
    if (!category.is_active) {
      throw new BadRequestException(`Category ${category.name} is currently inactive. Cannot build rooms under inactive categories.`);
    }
  ...
  async getRooms(search?: string, status?: string) {
    ...
    if (search) {
      query.andWhere('room.room_number ILIKE :search', { search: `%${search}%` });
    }
  ...
  async updateRoom(id: string, updateDto: UpdateRoomDto) {
    const room = await this.roomRepo.findOne({ where: { id }, relations: ['category'] });
    if (!room) {
      throw new NotFoundException(`Room with ID ${id} not found`);
    }

    if (updateDto.room_number && updateDto.room_number !== room.room_number) {
      const existing = await this.roomRepo.findOne({ where: { room_number: updateDto.room_number } });
      if (existing) {
        throw new ConflictException(`Room number ${updateDto.room_number} already exists`);
      }
